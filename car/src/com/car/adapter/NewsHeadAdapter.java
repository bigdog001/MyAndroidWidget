...
public class NewsHeadAdapter extends PagerAdapter
{
	...
	public Object instantiateItem(ViewGroup container, final int position)
	{
		...
		viewpager_item_img.setTag(ConstantsUtil.IMAGE_URL + datas.get(position % datas.size()).cover_image);
		// 开启异步加载图片,显示图片比例为screenW*200dp
		AsyncImageLoader.getInstance(context).loadBitmaps(layout, viewpager_item_img, ConstantsUtil.IMAGE_URL + datas.get(position % datas.size()).cover_image, LocalApplication.getInstance().screenW, DisplayUtil.dip2px(context, 200));
