...
public class CarSeriesAdapter extends SimpleBaseAdapter<CarSeries> implements PinnedHeaderAdapter, OnScrollListener
{
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		entityHolder.cover_list_item_content_name.setText(datas.get(position).alias_name);
		entityHolder.cover_list_item_content_price.setText(datas.get(position).dealer_price);

		// 给imageview设置一个tag，保证异步加载图片时不会乱序h
		entityHolder.cover_list_item_content_img.setTag(ConstantsUtil.IMAGE_URL + datas.get(position).alias_id + "_4.jpg");
		// 开启异步加载图片
		AsyncImageLoader.getInstance(c).loadBitmaps(view, entityHolder.cover_list_item_content_img, ConstantsUtil.IMAGE_URL + datas.get(position).alias_id + "_4.jpg");
